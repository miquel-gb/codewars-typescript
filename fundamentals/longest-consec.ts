...
export function longestConsec(strarr: string[], k: number): string {
    if (!strarr.length || k > strarr.length || k < 1) return '';
    let longestConcatStr = '';
    for (let i = 0; i <= strarr.length - k; i++) {
        //const concatStr = strarr.filter((val, index) => { return index >= i && index < i + k}).join('');
        const concatStr = strarr.slice(i, i + k).join('');
        if (concatStr.length > longestConcatStr.length) longestConcatStr = concatStr;
    }
